...
اگر
\m{|Z| \leq |Y| + |X|}
هر کدام از \m{X} یا \m{Z} که کوچک‌تر باشند، با \m{Y} ادغام می‌شوند (که الگوریتم ترجیح می‌دهد \m{X} را بخاطر اینکه در کش تازگی دارد را ادغام کند.)
...
\lr{\texttt{PyList\_Sort()}}
\fn{5}{\url{https://github.com/python/cpython/blob/3.10/Objects/listobject.c\#L2483}}
است که تابع پیاده‌سازی آن که تابع
\lr{\texttt{list\_sort\_impl()}}
\fn{6}{\url{https://github.com/python/cpython/blob/3.10/Objects/listobject.c\#L2221}}
را صدا می‌زند.
\end{itemize}
\end{frame}